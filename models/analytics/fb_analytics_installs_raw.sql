...
, user_pseudo_id_to_user_id AS (
        ...
        FROM {{ ref('fb_analytics_events_raw') }} WHERE user_id IS NOT NULL AND {{ ta_firebase.analyticsDateFilterFor('event_date', extend = 2) }}
        QUALIFY ROW_NUMBER() OVER (PARTITION BY user_pseudo_id ORDER BY event_ts) = 1
)

{%- set miniColumnsToIgnoreInGroupBy = ["duplicates_cnt"] -%}
{%- set columns = ta_firebase.get_filtered_columns_for_table("fb_analytics_events_raw", "*", miniColumnsToIgnoreInGroupBy)[0] -%}
...
{%- for column in columns %}
    ...
    {%- else -%}
        {%- set _ = columnsInSelect.append("COALESCE(custom." ~ column.name ~ ", ta." ~ column.name ~ ", fb." ~ column.name ~ ", anyFirstEvent." ~ column.name ~ ") as " ~ column.name) -%}
    {%- endif -%}
{%- endfor %}
...
, data as (
    ...
    LEFT JOIN user_pseudo_id_to_user_id as users ON COALESCE(anyFirstEvent.user_pseudo_id, fb.user_pseudo_id, ta.user_pseudo_id, custom.user_pseudo_id) = users.user_pseudo_id
    WHERE True 
)
